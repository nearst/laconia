...
describe('S3 Item Reader', () => {
  ...
  describe('when start reading in the middle', () => {
    ...
    it('generates nexts object with the correct content', async () => {
      expect(nexts[0]).toEqual({ item: 'Foo', cursor: { index: 3 }, finished: false })
      expect(nexts[1]).toEqual({ item: 'Bar', cursor: { index: 4 }, finished: false })
      expect(nexts[2]).toEqual({ item: 'Fiz', cursor: { index: 5 }, finished: true })
    })
  ...
  it('throws error when path given Body is not a JSON', async () => {
    s3.getObject.mockImplementation(yields({
      Body: { toString: () => 'boom' }
    }))
    const reader = new S3ItemReader(new AWS.S3(), s3Params, '.')
