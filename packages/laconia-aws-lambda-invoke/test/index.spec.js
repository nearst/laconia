...
describe('aws invoke', () => {
  ...
  describe('fire and forget', () => {
    it('should throw an error when FunctionError is set to Handled', () => {
      invokeMock.mockImplementation(yields({FunctionError: 'Handled', Payload: 'boom', StatusCode: 202}))
    ...
    it('should throw an error when FunctionError is set to Unhandled', () => {
      invokeMock.mockImplementation(yields({FunctionError: 'Unhandled', Payload: 'boom', StatusCode: 202}))
      const invoker = new LambdaInvoker(lambda, 'myLambda')
    ...
    describe('when invoking Lambda', () => {
      beforeEach(() => {
        invokeMock.mockImplementation(yields({FunctionError: undefined, StatusCode: 202}))
  ...
  describe('request response', () => {
    it('should throw an error when FunctionError is set to Handled', () => {
      invokeMock.mockImplementation(yields({FunctionError: 'Handled', Payload: 'boom', StatusCode: 200}))
      const invoker = new LambdaInvoker(lambda, 'myLambda')
      return expect(invoker.requestResponse()).rejects.toThrow('Handled error returned by myLambda: boom')
    })

    it('should throw an error when FunctionError is set to Unhandled', () => {
      invokeMock.mockImplementation(yields({FunctionError: 'Unhandled', Payload: 'boom', StatusCode: 200}))
      const invoker = new LambdaInvoker(lambda, 'myLambda')
      return expect(invoker.requestResponse()).rejects.toThrow('Unhandled error returned by myLambda: boom')
    })

    describe('when invoking Lambda', () => {
      beforeEach(() => {
        invokeMock.mockImplementation(yields({FunctionError: undefined, StatusCode: 200}))
        const invoker = new LambdaInvoker(lambda, 'foobar')
        return invoker.requestResponse({biz: 'baz'})
      })

      it('should set InvocationType parameter as RequestResponse', () => {
        expect(invokeMock).toBeCalledWith(
    ...
    it('should not set Payload parameter if it is not available', () => {
      invokeMock.mockImplementation(yields({FunctionError: undefined, StatusCode: 200}))
      const invoker = new LambdaInvoker(lambda, 'foobar')
      return invoker.requestResponse().then(_ => {
        const invokeParams = invokeMock.mock.calls[0][0]
        expect(invokeParams).not.toHaveProperty('Payload')
      })
    })
